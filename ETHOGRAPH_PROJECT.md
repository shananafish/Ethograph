...
**Note (flow restructure, 2026-07-30):** the **Select Group** screen is now group-pick only (`buildFilterUI` builds type + group chips; tapping a group → `pickGroup` → `applyRosterFilter()` → home hub). The **per-session animal picker moved to the Setup screen** (`buildAnimalPicker` = `buildDemogUI`+`buildIndividualUI`, called from `populateSetupForm`; `#filter-demogs`/`#filter-individuals`/`#filter-count` markup relocated there). `startSession` calls `applyRosterFilter(false)` to rebuild `INDS` from the chosen subset before starting. `applyRosterFilter(dest)` takes an optional destination (`false` = no nav). The **Notes** page (view id still `dashboard`, titled "Notes") is reached from home (general — `Session = n/a`) and from inside a session via a **📝 Notes** button on the session/recording headers (tagged with the session); `openNotes`/`notesBack` remember the origin view. A **↑ Sync Notes** button (`syncNotesOnly` → `postNotesToSheet`) syncs only the notes tab. No-session notes export `Session = n/a`.
